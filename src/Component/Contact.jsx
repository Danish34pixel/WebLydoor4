...
const StatBadge = ({ value, suffix, label, delay, inView }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-1"
      ...
      <span className="text-3xl font-extrabold text-[#C4F20D] font-['Eurostile'] tracking-wider"
    ...
    >
      ...
      <div className="relative w-12 h-12 shrink-0 flex items-center justify-center">
        ...
        <div className="w-full h-full rounded-full bg-[#C4F20D]/10 border border-[#C4F20D]/30 flex items-center justify-center"
          style={{
            boxShadow: hovered ? '0 0 22px rgba(196,242,13,0.4)' : '0 0 10px rgba(196,242,13,0.1)',
      ...
      <div>
        <p className="text-xs text-[#C4F20D]/50 font-bold tracking-[0.15em] uppercase font-['Eurostile'] mb-1">{label}</p>
...
const InputField = ({ label, type = "text", placeholder, value, onChange, name, animDelay = 0, formVisible }) => {
  ...
  return (
    <div className="relative flex flex-col gap-2"
      style={{
        opacity: formVisible ? 1 : 0, transform: formVisible ? 'translateY(0)' : 'translateY(20px)',
        transition: `opacity 0.6s ease ${animDelay}ms, transform 0.6s cubic-bezier(0.16,1,0.3,1) ${animDelay}ms`
      }}>
      <label className="text-xs font-bold tracking-[0.18em] uppercase font-['Eurostile']"
    ...
    <button ref={btnRef} type="submit" disabled={sending}
      ...
      {ripples.map(rp => (
        <span key={rp.id} className="absolute rounded-full bg-black/15 pointer-events-none"
          style={{
            left: rp.x, top: rp.y, width: 0, height: 0, transform: 'translate(-50%,-50%)',
      ...
      {sending ? (
        ...
        <span className="flex items-center justify-center gap-2 font-['Eurostile']">
          Send Message
...
const StarIcon = () => (
  <svg className="w-3.5 h-3.5 text-[#C4F20D]" fill="currentColor" viewBox="0 0 20 20">
    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
  </svg>
);

const ReviewCard = ({ name, role, text, stars }) => (
  <div className="flex-shrink-0 w-72 sm:w-80 mx-3 bg-[#0a100a]/80 backdrop-blur-xl border border-[#C4F20D]/15 rounded-2xl p-5 relative overflow-hidden"
    style={{ boxShadow: '0 4px 30px rgba(0,0,0,0.4), inset 0 0 0 1px rgba(196,242,13,0.06)' }}>
    {/* Subtle glow top corner */}
    <div className="absolute top-0 right-0 w-20 h-20 rounded-full pointer-events-none"
      style={{ background: 'radial-gradient(circle, rgba(196,242,13,0.08) 0%, transparent 70%)', transform: 'translate(30%, -30%)' }} />
    {/* Stars */}
    <div className="flex gap-0.5 mb-3">
      {Array.from({ length: stars }).map((_, i) => <StarIcon key={i} />)}
    </div>
    {/* Quote */}
    <p className="text-gray-300 text-sm font-sans leading-relaxed mb-4 line-clamp-3">{text}</p>
    {/* Author */}
    <div className="flex items-center gap-3">
      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#C4F20D]/30 to-[#C4F20D]/10 border border-[#C4F20D]/30 flex items-center justify-center flex-shrink-0">
        <span className="text-[#C4F20D] text-xs font-extrabold font-['Eurostile']">{name.charAt(0)}</span>
      </div>
      <div>
        <p className="text-white text-xs font-bold tracking-wide">{name}</p>
        <p className="text-[#C4F20D]/50 text-[10px] tracking-widest uppercase">{role}</p>
      </div>
    </div>
  </div>
);
...
const FloatingReviews = () => {
  ...
  return (
    <div className="relative w-full overflow-hidden my-10 py-2">
      {/* Fade edges */}
      ...
      <div
        className="flex"
        style={{ animation: 'reviewsScroll 35s linear infinite', width: 'max-content' }}
      >
        {doubled.map((r, i) => (
          <ReviewCard key={i} {...r} />
        ))}
      ...
      <section ref={sectionRef}
        ...
        <div className="max-w-7xl mx-auto w-full relative z-10">
          ...
          <div ref={headerRef} className="text-center mb-14 sm:mb-20"
            style={{
              opacity: headerInView ? 1 : 0, transform: headerInView ? 'translateY(0)' : 'translateY(36px)',
            ...
            <h2 className="text-4xl sm:text-5xl lg:text-7xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[#C4F20D] to-white font-['Eurostile'] tracking-wider mb-5 title-glitch">
              Get In Touch
            </h2>
            ...
            {/* Stats */}
            <div ref={statsRef} className="stats-row flex items-center justify-center gap-10 sm:gap-16 mt-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-start">
            ...
            <div className="lg:col-span-5 flex flex-col gap-6">
              <div style={{
                opacity: headerInView ? 1 : 0, transform: headerInView ? 'translateX(0)' : 'translateX(-24px)',
              ...
              <div className="flex flex-col gap-4 mt-2">
                <InfoCard delay={0} label="Email" value="weblydoor.com"
                ...
                />
                <InfoCard delay={100} label="Phone" value="+91 8299568058"
                ...
                <div className="flex items-center gap-4 mt-2" style={{
                  ...
                  transition: 'opacity 0.8s ease 350ms, transform 0.8s cubic-bezier(0.16,1,0.3,1) 350ms'
                }}>
                  {[
                    { name: 'WhatsApp', url: 'https://wa.me/918299568058', icon: <path stroke="none" fill="currentColor" d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 0 0-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413Z" /> },
                    { name: 'LinkedIn', url: '#', icon: <><path d="M16 8a6 6 0 016 6v7h-4v-7a2 2 0 00-2-2 2 2 0 00-2 2v7h-4v-7a6 6 0 016-6zM2 9h4v12H2z" /><circle cx="4" cy="4" r="2" /></> },
                    ...
                    { name: 'Instagram', url: '#', icon: <><rect x="2" y="2" width="20" height="20" rx="5" ry="5" /><path d="M16 11.37A4 4 0 1112.63 8 4 4 0 0116 11.37zm1.5-4.87h.01" /></> },
                    { name: 'Facebook', url: '#', icon: <path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z" /> }
                  ].map((social, idx) => (
                    <a key={idx} href={social.url} target="_blank" rel="noopener noreferrer" 
                       className="w-11 h-11 rounded-full bg-[#C4F20D]/5 border border-[#C4F20D]/20 flex items-center justify-center text-[#C4F20D]/70 hover:text-[#020500] hover:bg-[#C4F20D] hover:scale-110 hover:shadow-[0_0_20px_rgba(196,242,13,0.4)] transition-all duration-300 group"
                       title={social.name}>
                      <svg className="w-5 h-5 group-hover:scale-110 transition-transform duration-300" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
                        {social.icon}
                      </svg>
                    </a>
                  ))}
              ...
              <div className="relative rounded-2xl p-[1px] overflow-hidden mt-2"
                style={{
                  background: 'linear-gradient(90deg, rgba(196,242,13,0.6) 0%, rgba(196,242,13,0.1) 50%, rgba(196,242,13,0.6) 100%)',
                ...
                <div className="relative rounded-2xl overflow-hidden bg-[#050702]/85 backdrop-blur-xl p-7">
                  ...
                  <h3 className="text-xl sm:text-2xl font-extrabold text-[#C4F20D] font-['Eurostile'] tracking-wider mb-3 relative z-10">
            ...
            <div ref={formRef}
              ...
              style={{
                opacity: formInView ? 1 : 0, transform: formInView ? 'translateY(0)' : 'translateY(44px)',
              ...
              {[
                ...
                <div key={i} className={`absolute ${cls} border-[#C4F20D] pointer-events-none`}
                  style={{
                    width: formInView ? (i === 0 ? 72 : 52) : 0, height: formInView ? (i === 0 ? 72 : 52) : 0,
              ...
              <div className="absolute bottom-4 right-6 text-8xl font-extrabold font-['Eurostile'] select-none pointer-events-none"
              ...
              {!submitted ? (
                <>
                  <div className="mb-7 relative z-10"
                    style={{
                      opacity: formInView ? 1 : 0, transform: formInView ? 'translateY(0)' : 'translateY(16px)',
                      transition: 'opacity 0.6s ease 200ms, transform 0.6s ease 200ms'
                    }}>
                    <h3 className="text-2xl sm:text-3xl font-extrabold text-white font-['Eurostile'] tracking-wider mb-2">
                      Send Us a Message
                    </h3>
                ...
                <div className="flex flex-col items-center justify-center py-14 gap-7 relative z-10 success-pop">
                  ...
                  <div className="relative flex items-center justify-center">
                    {[0, 1, 2].map(i => (
                      <div key={i} className="absolute rounded-full border border-[#C4F20D]"
                        style={{
                          width: 80 + i * 36, height: 80 + i * 36, opacity: 0.15 - i * 0.04,
                  ...
                  <div className="text-center">
                    <h3 className="text-2xl font-extrabold text-[#C4F20D] font-['Eurostile'] tracking-wider mb-3">Message Sent!</h3>
                  ...
                  <button onClick={() => { setSubmitted(false); setForm({ name: '', email: '', phone: '', message: '' }); }}
                    className="text-xs text-[#C4F20D]/60 hover:text-[#C4F20D] underline underline-offset-4 tracking-widest uppercase font-bold transition-colors font-['Eurostile']">
                    Send Another
                  </button>
                </div>
              )}
        ...
        {/* Footer Text */}
        <div className="w-full mt-24 pt-8 border-t border-[#C4F20D]/10 text-center relative z-10">
          <p className="text-gray-400 font-sans text-sm md:text-base tracking-widest uppercase">
            Designed & Developed by <span className="text-[#C4F20D] font-bold font-['Eurostile']">Weblydoor</span>
          </p>
        </div>
      </section>
    </>
